...
command! -bang -complete=file -nargs=* FZFGit call fzf#run({ 'source':  printf('(cd %s ; git ls-files --no-empty-directory --exclude-standard . | egrep -v "\.zip$|\.gz$|\.jpg$|\.png$|\.jar$" | sed -e s,^,%s/,)', escape(empty(<q-args>) ? '.' : <q-args>, '"\'), escape(empty(<q-args>) ? '.' : <q-args>, '"\')), 'sink':    'edit', 'options': '-m -x +s -e', 'down':    '40%' })
command! FZFMru call fzf#run({ 'source':  reverse(s:all_files()), 'sink':    'edit', 'options': '-m -x +s -e', 'down':    '40%' })
command! FZFFavorites call fzf#run({ 'source':  'fdfind -H -a --type file --color never . ${HOME}/dotfiles/ ${HOME}/all-things-phd', 'sink':    'edit', 'options': '-m -x +s -e', 'down':    '40%' })
command! -range -nargs=1 AddNumbers call s:add_numbers((<line2>-<line1>+1) * eval(<args>))
command! FZFLines mksession! /tmp/layout.vim | call fzf#run({   'source':  <sid>buffer_lines(),   'sink':    function('<sid>line_handler'),   'options': '--extended --nth=3..',   'down':    '60%'})
command! Wa wa
command! WQa wqa
command! Qa qa
command! Wqa wqa
...
function! s:add_numbers(num)
  ...
  if l:prev_num !=? ''
    ...
    let l:new_line = l:prev_line[: -len(l:prev_num)-1] . printf('%0'.len(l:prev_num . l:next_num).'d',    max([0, l:prev_num . l:next_num + a:num])) . l:next_line[len(l:next_num):]
  else
    let l:new_line = l:prev_line . substitute(l:next_line, '\d\+', "\\=printf('%0'.len(submatch(0)).'d',         max([0, submatch(0) + a:num]))", '')
  endif
...
function! s:all_files()
  let l:oldfilesFiltered = filter(copy(v:oldfiles),        "v:val !~# 'fugitive:\\|NERD_tree\\|^/tmp/\\|.git/|.REMOTE.*'")
  let l:indexedFileList = extend(l:oldfilesFiltered, map(filter(range(1, bufnr('$')), 'buflisted(v:val)'), 'bufname(v:val)'))
...
function! s:ag_to_qf(line)
  ...
  return {'filename': l:parts[0], 'lnum': l:parts[1], 'col': l:parts[2], 'text': join(l:parts[3:], ':')}
endfunction
...
function! s:ag_handler(lines)
  ...
  let l:cmd = get({'ctrl-x': 'split', 'ctrl-v': 'vertical split', 'ctrl-t': 'tabe'}, a:lines[0], 'e')
...
command! -complete=file -nargs=* Ag mksession! /tmp/layout.vim | call fzf#run({ 'source':  printf('ag --nogroup --column --nocolor "^(?=.)" "%s"',                   escape(empty(<q-args>) ? '.' : <q-args>, '"\')), 'sink*':    function('<sid>ag_handler'), 'options': '--ansi --expect=ctrl-t,ctrl-v,ctrl-x --delimiter : --nth 4.. '.            '--multi --bind ctrl-a:select-all,ctrl-d:deselect-all '.            '--color hl:68,hl+:110', 'down':    '50%' })
...
let &titlestring=" %{expand('%:p:.:~')}%(%m%r%w%) %<\(%{".s:SID_PREFIX()."strwidthpart(fnamemodify(getcwd(), ':~'), &columns-len(expand('%:p:.:~')))}\) - VIM"
let &statusline=' ' . "%{(&previewwindow?'[preview] ':'').expand('%')}" . "%=%m%y%{'['.(&fenc!=''?&fenc:&enc).','.&ff.']'}" . "%{printf(' %4d/%d',line('.'),line('$'))} %c"
