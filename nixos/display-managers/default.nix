...
let
  ...
  cfg = config.services.customXServer;
  xorg = pkgs.xorg;
  ...
  # file provided by services.customXServer.displayManager.session.script
...

{
  ...
  options = {
    ...
    services.customXServer.displayManager = {
  ...
  config = {
    services.customXServer.displayManager.xserverBin = "${xorg.xorgserver.out}/bin/X";
  };

  imports = [
   (mkRemovedOptionModule [ "services" "customXServer" "displayManager" "desktopManagerHandlesLidAndPower" ]
